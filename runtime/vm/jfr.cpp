...
createNewEventWriter(J9VMThread *currentThread)
{
	...
	eventWriter = jvmUpcallsConstructorEventWriter(currentThread, (UDATA)currentThread->jfrJavaEventBuffer.bufferCurrent, (UDATA)currentThread->jfrJavaEventBuffer.bufferStart + J9JFR_THREAD_BUFFER_SIZE, (UDATA)currentThread->jfrJavaEventBuffer.bufferStart, getThreadTID(currentThread, currentThread), TRUE);
	if (NULL == eventWriter) {
		vmFuncs->setNativeOutOfMemoryError(currentThread, 0, 0);
		goto done;
	}
...
initializeEventWriterFields(J9VMThread *currentThread)
{
	...
	J9Class *eventWriterClass = vmFuncs->internalFindClassUTF8(currentThread, (U_8 *)J9UTF8_DATA(&jfrEventWriterClassUTF8), J9UTF8_LENGTH(&jfrEventWriterClassUTF8), vm->systemClassLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
...
flushJavaJFRBuffer(J9VMThread *currentThread, jobject eventWriterRef, I_32 uncommited, I_32 needed)
{
	...
	if (newEventSize > J9JFR_THREAD_BUFFER_SIZE) {
		...
		currentThread->jfrJavaEventBuffer.bufferRemaining = roundedNewEventSize;
